...
//                 copy-paste of the same function in custom_call_thunks.cc
...
absl::Span<const int64_t> DecodeDims(int64_t* encoded_dims_data) {
  const auto dims_count = encoded_dims_data[0];
  const auto dims_begin = encoded_dims_data + 1;
...
void BuildBuffers(absl::Span<const int32_t> types, int64_t* encoded_dims,
  ...
  for (int64_t i = 0; i < types.size(); ++i) {
    ...
    const auto data_width = xla::primitive_util::ByteWidth(dtype) * elem_count;
