...
bool IsSingleDevice(
  ...
  return compile_metadata.num_replicas() *
             compile_metadata.num_cores_per_replica() ==
         1;
}
...
IfrtServingExecutable::CreateExecutableSynchronously(
  ...
  TF_ASSIGN_OR_RETURN(Tf2HloResult tf2hlo_result,
                      CompileTfToHlo(compile_metadata, *module_,
                                     dtypes_and_shapes, signature_name(),
...
IfrtServingExecutable::LookUpOrCreateExecutable(
  ...
  for (const auto& computation_device :
       compile_metadata.device_assignment().computation_devices()) {
    device_ids.insert(computation_device.replica_device_ids().begin(),
                      computation_device.replica_device_ids().end());
  }
  Key key = {.device_ids = device_ids, .input_shapes = input_shapes};
...
absl::StatusOr<std::vector<tensorflow::Tensor>> IfrtServingExecutable::Execute(
  ...
  if (ifrt_serving_core_selector_) {
    device_reservation =
  ...
  TF_ASSIGN_OR_RETURN(tensorflow::tpu::TPUCompileMetadataProto compile_metadata,
                      GetCompileMetadata(*module_, dtypes_and_shapes,
                                         *ifrt_client_, device_reservation));
